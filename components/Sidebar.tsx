  ...
  return (
    <div className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col fixed left-0 top-0 z-10">
      ...
      <div className="p-6 flex justify-center items-center">
        <div className="w-16 h-16 bg-orange-100 rounded-lg flex items-center justify-center border-2 border-orange-400">
           ...
           <div className="text-center">
             <div className="text-[10px] font-bold tracking-widest text-gray-800">JUST</div>
             <div className="text-3xl font-bold text-orange-500 leading-none">M</div>
           </div>
        </div>
      </div>
        ...
        <div>
          <button 
            ...
            className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-colors duration-200 ${
              isSettingsActive ? 'text-indigo-600' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
          ...
          {isSettingsOpen && (
            <div className="mt-1 space-y-1">
              <NavLink to="/settings/general" className={subNavItemClass('/settings/general')}>
                General Setting
              </NavLink>
              <NavLink to="/settings/kyc" className={subNavItemClass('/settings/kyc')}>
                KYC Setting
              </NavLink>
            </div>
          )}
      ...
      <div className="p-4 border-t border-gray-100 absolute bottom-0 w-full bg-white">
        <button className="flex items-center justify-center gap-2 w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg transition-colors shadow-sm">
          <LogOut size={18} />
          <span>Log Out</span>
        </button>
      </div>
